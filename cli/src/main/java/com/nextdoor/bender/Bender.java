...
public class Bender {
  ...
  // Global defaults that are not yet overridable, but one day may be configurable
  // on the CLI.
  private static final String AWS_REGION = "us-east-1";
  private static final String AWS_ACCOUNT = "123456789";

  /**
   * Main entrypoint for the Bender CLI tool - handles the argument parsing
   * and triggers the appropriate methods for ultimately invoking a Bender
   * Handler.
   *
   * @param args
  ...
  public static void main(String[] args) throws ParseException {
    ...
    Option help = Option.builder("H")
            ...
            .build();
    Option handler = Option.builder("h")
            .longOpt("handler")
            .hasArg()
            ...
            .build();
    Option source_file = Option.builder("s")
            .longOpt("source_file")
            .required()
            .hasArg()
            ...
            .build();
    Option kinesis_stream_name = Option.builder()
            .longOpt("kinesis_stream_name")
            .hasArg()
            .desc("What stream name should we mimic? " +
    ...
    // The CLI tool doesn't have any configuration files built into it. We
    // require that the user set BENDER_CONFIG to something reasonable.
    if (System.getenv("BENDER_CONFIG") == null) {
      logger.error(
              "You must set the BENDER_CONFIG environment variable. \n" +
    ...
    // Depending on the desired Handler, we invoke a specific method and pass in the
    // options (or defaults) required for that handler.
  ...
  protected static void invokeKinesisHandler(String stream_name, String source_file) throws HandlerException {
    ...
    // Create a series of KinesisEvents from the source file. All of these events will be treated
    // as a single batch that was pushed to Kinesis, so they will all have the same Arrival Time.
    ...
    while(scan.hasNextLine()){
      ...
      rec.withPartitionKey("1")
              .withSequenceNumber(r + "")
    ...
    logger.info("Processed " + r + " records");

    // Create the main Kinesis Event object - this holds all of the data and records
    // that will be passed into the Kinesis Handler.
  ...
  /**
   * Generates an Amazon TestContext object that will be used to invoke our Bender
   * Handlers.
   *
   * @return TestContext
   */
